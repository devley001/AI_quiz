...
BACKEND_URL = "http://localhost:5000/api"  # Adjust as needed
...
def initialize_adaptive_session():
    ...
    try:
        ...
        # Store session data in backend database
        try:
            session_data = {
                'sessionId': result['session_id'],
                'userId': data['user_id'],
                'topic': data['topic'],
                'initialAbility': result['initial_ability'],
                'initialBloomsLevel': result['initial_level'],
                'status': 'active'
            }
            backend_response = requests.post(f"{BACKEND_URL}/adaptive/session", json=session_data)
            if backend_response.status_code != 201:
                logger.warning(f"Failed to store session in backend: {backend_response.text}")
        except Exception as e:
            logger.error(f"Error storing session in backend: {str(e)}")
...
def get_next_adaptive_question():
    ...
    try:
        ...
        # Get question bank from backend
        try:
            backend_response = requests.get(f"{BACKEND_URL}/questions/bank/{data.get('topic', 'general')}")
            if backend_response.status_code == 200:
                question_bank = backend_response.json().get('data', [])
            else:
                question_bank = _get_sample_question_bank()
        except Exception as e:
            logger.error(f"Error fetching question bank from backend: {str(e)}")
            question_bank = _get_sample_question_bank()
    ...
    try:
        ...
        try:
            update_data = {
                ...
                'updatedAbility': result['updated_ability'],
                'currentBloomsLevel': result['current_blooms_level'],
                'performanceMetrics': result['performance_metrics']
            }
            backend_response = requests.put(f"{BACKEND_URL}/adaptive/session/update", json=update_data)
            if backend_response.status_code != 200:
                logger.warning(f"Failed to update session in backend: {backend_response.text}")
        except Exception as e:
            logger.error(f"Error updating session in backend: {str(e)}")
...
def classify_question_taxonomy():
    ...
    try:
        ...
        # Store classification in backend
        try:
            classification_data = {
                'questionId': data.get('question_id', f"classified_{hash(data['question_text'])}"),
                'cognitiveLevel': result['cognitive_level'],
                'levelIndex': result['level_index'],
                'confidence': result['confidence'],
                'detectedVerbs': result['detected_verbs'],
                'questionText': data['question_text'],
                'topic': data.get('topic', 'general')
            }
            backend_response = requests.post(f"{BACKEND_URL}/blooms/classify", json=classification_data)
            if backend_response.status_code != 201:
                logger.warning(f"Failed to store classification in backend: {backend_response.text}")
        except Exception as e:
            logger.error(f"Error storing classification in backend: {str(e)}")
...
def calibrate_item_parameters():
    ...
    try:
        ...
        # Update IRT parameters in backend
        try:
            irt_data = {
            ...
            backend_response = requests.put(f"{BACKEND_URL}/irt/calibrate", json=irt_data)
            if backend_response.status_code != 200:
                logger.warning(f"Failed to update IRT parameters in backend: {backend_response.text}")
        except Exception as e:
            logger.error(f"Error updating IRT parameters in backend: {str(e)}")
